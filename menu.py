...
def game(difficulty, music_volume, game_volume):
    pygame.mixer.music.stop()
    Player.Start(difficulty, music_volume, game_volume, current_music_index)
    pygame.quit()
    sys.exit()
...
def main_menu():
    ...
    if os.path.exists(SETTINGS_FILE):
        with open(SETTINGS_FILE, "r") as f:
            try:
                ...
                game_volume = settings.get("game_volume", 0.5)
                difficulty = settings.get("difficulty", "Легко")
            except json.JSONDecodeError:
                print("Ошибка при чтении файла настроек, использованы значения по умолчанию.")
    ...
    load_and_play_music()
    play_button = Button(SCREEN_WIDTH // 2 - 100, SCREEN_HEIGHT // 2 - 50, 200, 50, "Играть", WHITE, LIGHT_BLUE, lambda: game(difficulty, music_volume, game_volume)) #кнопки стали белыми
